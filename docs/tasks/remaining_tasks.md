...
- [ ] 데이터베이스 마이그레이션 실행 (Dashboard에서 수동 실행 필요)
- [ ] Edge Functions 배포
- [ ] RLS 정책 테스트

### 1.2 52시간 근무제 완성
- [ ] 주간 근무시간 실시간 계산 트리거 검증
- [ ] 48시간 경고 알림 시스템 구현
- [ ] 52시간 차단 로직 테스트
- [ ] 관리자 대시보드 경고 UI
...
#### 컴포넌트 개발
- [ ] `components/dashboard/StatsCards.tsx` - 통계 카드
- [ ] `components/dashboard/RealtimeAttendance.tsx` - 실시간 출퇴근 현황
- [ ] `components/dashboard/WeeklyHoursChart.tsx` - 주간 근무시간 차트
- [ ] `components/attendance/ClockInButton.tsx` - 출퇴근 버튼
- [ ] `components/attendance/AttendanceCalendar.tsx` - 근태 캘린더
- [ ] `components/attendance/OvertimeAlert.tsx` - 초과근무 알림

#### 페이지 완성
- [ ] `app/(auth)/signup/page.tsx` - 회원가입
- [ ] `app/(auth)/forgot-password/page.tsx` - 비밀번호 찾기
- [ ] `app/attendance/page.tsx` - 출퇴근 관리
- [ ] `app/attendance/clock/page.tsx` - 출퇴근 체크
- [ ] `app/schedule/page.tsx` - 근무일정 관리
- [ ] `app/leave/page.tsx` - 휴가 관리
- [ ] `app/leave/request/page.tsx` - 휴가 신청
- [ ] `app/employees/page.tsx` - 직원 관리 (관리자)
- [ ] `app/reports/page.tsx` - 리포트

#### API Routes
- [ ] `app/api/auth/[...supabase]/route.ts` - Supabase Auth 핸들러
- [ ] `app/api/webhooks/kakao/route.ts` - 카카오톡 웹훅

### 2.2 Mobile App (React Native)

#### 화면 구현
- [ ] `screens/Auth/LoginScreen.tsx` - 로그인
- [ ] `screens/Auth/BiometricSetup.tsx` - 생체인증 설정
- [ ] `screens/Home/HomeScreen.tsx` - 홈 화면
- [ ] `screens/Home/QuickActions.tsx` - 빠른 작업
...
#### 네비게이션
- [ ] `navigation/AppNavigator.tsx` - 앱 네비게이터
...
*마지막 업데이트: 2024-12-28 (Supabase 프로젝트 설정 완료)*
